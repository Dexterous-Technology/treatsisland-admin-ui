...
import JoinEventHelper from "../../../join-event/join-event-helper";
import "./quick-wizard-auth.scss";
import QuickWizardEmail from "../quick-wizard-email/quick-wizard-email";
...
const TermsContent = ({
  ...
  return (
    <div className={"termsPopup " + (shouldShow ? "show" : "")}>
      <div className="heading">
        <div className="h3">Terms & Condition</div>
      ...
      <div className="innerContent">
        <p>
          {" "}
          This website is operated by Treats Island Candy . Throughout the site,
          the terms “we”, “us” and “our” refer to Treats Island Candy and Hall
          Retail Enterprises, LLC . Treats Island Candy offers this website,
        ...
        <p>
          ...
          also be subject to the Terms and Conditions or Terms of Service. You
          can review the most current version of the Terms and Conditions at any
        ...
        <p>
          Our store is hosted on AWS. They provide us with the online e-commerce
        ...
        <p>
          In no case shall Treats Island Candy , our directors, officers,
        ...
        <p>
          You agree to indemnify, defend and hold harmless Treats Island Candy
        ...
        <p>
          Questions about the Terms of Service should be sent to us via email at
          Hallretail1@gmail.com.
        </p>
        <p>Our office contact information is posted below:</p>
        <p>
          Treats Island Candy <br />
          ℅ Hall Retail Enterprises, LLC <br />
          10808 Foothill Blvd. Suite 160-580 A Rancho Cucamonga, CA 91730
        </p>
      </div>

      <div className="buttons">
        <span className="button primary" onClick={onAccept}>
          Accept
        </span>
...
const QuickWizardAuth = ({
  ...
  const [verifierInstance, setVerifierInstance] = useState(null);
  const [otpInput, setOtpInput] = useState("");
  const [errorMessage, setErrorMessage] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [isTermsChecked, setIsTermsChecked] = useState(false);
  const [activeInputType, setActiveInputType] = useState("phone"); // Other option: ['otp', 'email']
  const history = useHistory(false);

  const _submitPhoneNumber = async () => {
    setErrorMessage("");
    if (phoneInput?.length >= 10) {
      let formattedInput = phoneInput;
      formattedInput.replace("+1", "");
      formattedInput = `+1${formattedInput}`;
      try {
        setIsLoading(true);
        const result = await AuthHelper.sendOtp(formattedInput);
        if (result.success) {
          setVerifierInstance(result.response);
          setActiveInputType("otp");
        }
      } catch (error) {
  ...
  const _onAuthSuccess = () => {
    if (onlyLogin) {
      if (eventCodeToJoin?.length) {
        JoinEventHelper.initiateJoinProcess({
          eventId: eventCodeToJoin,
          historyRef: history,
        });
      } else {
  ...
  const _submitOtp = async () => {
    ...
    if (otpInput?.length === 6) {
      try {
        setIsLoading(true);
        const result = await AuthHelper.loginWithOtp({
        ...
        if (result.success) {
          ...
          setErrorMessage("OTP didn't match");
        }
      } catch (error) {
      ...
      setErrorMessage("Please provide a valid 6 digit otp");
    }
  };
  ...
  return (
    <>
      {activeInputType === "email" ? (
        <>
          <QuickWizardEmail onSubmit={_updateEmail} />
        </>
      ) : (
        <>
                ...
                <>
                  <div className="form-group text-center">
                    ...
                    <label htmlFor="" className="h4 font-weight-bold">
                      Add Mobile +1{phoneInput.replace("+1", "")} for code (OTP)
                    </label>
                    {/* <input className="form-control" placeholder="Enter your phone number" type="text" /> */}
                  ...
                  <span
                    className="font-weight-bold text-danger ml-1 resend"
                    onClick={(e) => setotpField(false)}
                  >
                    RESEND OTP
                  </span>
                </div> */}
                  </div>
                  <div className="wizard_buttons text-center auth_button">
                    ...
                    >
                      {isLoading ? "Please wait" : "Verify and next"}
                    </span>
                  </div>
                </>
              ) : (
                <>
                  <div className="form-group text-center">
                    ...
                    <div className="small my-2">
                      We'll send you an One Time Password on this mobile number
                    </div>
                    <input
                      disabled={isLoading}
                      className="form-control"
                      placeholder="Add mobile phone for code (OTP)"
                      type="text"
                      value={phoneInput}
                  ...
                  <div
                    className="agree-to-terms text-center mt-2 mb-4"
                  ...
                  >
                    ...
                    <TermsContent
                      shouldShow={showTerms}
                      onAccept={(e) => {
                        setShowTerms(false);
                        setIsTermsChecked(true);
                      }}
                      onDismiss={(e) => setShowTerms(false)}
                    />
                  </div>
...
export default QuickWizardAuth;
